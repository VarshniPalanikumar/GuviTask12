...
def test_Resources(start_browser):
    ...
    fourth_element = WebDriverWait(driver,10).until(
        EC.presence_of_element_located((By.XPATH,"//div[@id='solutions']//p[normalize-space()='Resources']"))
    )
    print(fourth_element.get_attribute("textContent"))

    #Parent of Resources element
    Parent_Resourse = WebDriverWait(driver,10).until(
        EC.presence_of_element_located((By.XPATH,"//div[@id='solutions']//p[normalize-space()='Resources']/parent::*"))
    )
    assert Parent_Resourse is not None

    #Child of Resource
    Child_Resourse = WebDriverWait(driver,10).until(
        EC.presence_of_element_located((By.XPATH,"//div[@id='solutions']//p[normalize-space()='Resources']/parent::*/child::*[1]"))
    )
    assert Child_Resourse is not None

    #Locate Second sibling of resource
    second_sibling = WebDriverWait(driver,10).until(
        EC.presence_of_element_located((By.XPATH,"//div[@id='solutions']//p[normalize-space()='Resources']/following-sibling::*"))
    )
    ...
    All_Ancestors_resources = WebDriverWait(driver,10).until(
        EC.presence_of_all_elements_located((By.XPATH,"//div[@id='solutions']/p[contains(text(),'Resources')]/ancestor::*"))
    )
    assert All_Ancestors_resources is not None

    #following all siblings
    sibling_resources = WebDriverWait(driver,10).until(
        EC.presence_of_element_located((By.XPATH,"//div[@id='solutions']/p[contains(text(),'Resources')]/following-sibling::*"))
    )
    assert sibling_resources is not None

    #all precedings
    all_preceding = WebDriverWait(driver,10).until(
        EC.presence_of_all_elements_located((By.XPATH,"//div[@id='solutions']/p[contains(text(),'Resources')]/preceding::div[@class='⭐️f6lmuc-0 group relative cursor-pointer']"))
    )
